...
| <a name="input_image"></a> [image](#input\_image) | Talos image configuration | <pre>object({<br/>    schematic_path        = string<br/>    version               = string<br/>    arch                  = optional(string, "amd64")<br/>    factory_url           = optional(string, "https://factory.talos.dev")<br/>    platform              = optional(string, "nocloud")<br/>    datastore             = optional(string, "local")<br/>    update_schematic_path = optional(string)<br/>    update_version        = optional(string)<br/>  })</pre> | n/a | yes |
| <a name="input_nodes"></a> [nodes](#input\_nodes) | Configuration for cluster nodes | <pre>map(object({<br/>    cpu                  = number<br/>    host_node            = string<br/>    ip                   = string<br/>    machine_type         = string<br/>    ram_dedicated        = number<br/>    vm_id                = number<br/>    bridge               = optional(string, "vmbr0")<br/>    cpu_type             = optional(string, "x86-64-v2-AES")<br/>    datastore            = optional(string, "local-zfs")<br/>    disk_size            = optional(number, 20)<br/>    dns                  = optional(list(string))<br/>    igpu                 = optional(bool, false)<br/>    mac_address          = optional(string, null)<br/>    talos_config_patches = optional(list(string), [])<br/>    update               = optional(bool, false)<br/>    vlan_id              = optional(number, 0)<br/>  }))</pre> | n/a | yes |
...
| <a name="input_talos_config_patches"></a> [talos\_config\_patches](#input\_talos\_config\_patches) | Paths to additional Talos Machine Configuration patches | <pre>list(<br/>    object({<br/>      path         = string<br/>      machine_type = optional(string, "all") # "all", "controlplane", "worker"<br/>    })<br/>  )</pre> | `[]` | no |
| <a name="input_volumes"></a> [volumes](#input\_volumes) | Additional storage volumes available to the cluster | <pre>map(<br/>    object({<br/>      # common<br/>      size = string                          # in GB (not GiB)<br/>      type = optional(string, "proxmox-csi") # "directory", "disk", "partition", "proxmox-csi"<br/>      # directory, disk and partition<br/>      machine_type = optional(string, "worker") # "all", "controlplane", "worker"<br/>      # disk and proxmox-csi<br/>      datastore = optional(string) # default is to use the main VM's datastore  <br/>      # proxmox-csi only<br/>      node   = optional(string)<br/>      format = optional(string, "raw")<br/>      vmid   = optional(number, 9999)<br/>    })<br/>  )</pre> | `{}` | no |
