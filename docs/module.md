...
| <a name="input_volumes"></a> [volumes](#input\_volumes) | Additional storage volumes available to the cluster | <pre>map(<br/>    object({<br/>      # common<br/>      size = string                          # in GB (not GiB)<br/>      type = optional(string, "proxmox-csi") # "directory", "disk", "partition", "proxmox-csi"<br/>      # directory, disk and partition<br/>      machine_type = optional(string, "worker") # "all", "controlplane", "worker"<br/>      # disk and proxmox-csi<br/>      datastore = optional(string, "local-zfs")<br/>      # proxmox-csi only<br/>      node   = optional(string)<br/>      format = optional(string, "raw")<br/>      vmid   = optional(number, 9999)<br/>    })<br/>  )</pre> | `{}` | no |
